...
router.post('/login', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // NUEVO: además de responder el token en JSON, lo guardamos en cookie HttpOnly
    try {
    ...
    } catch { /* si no hay cookie-parser, no rompe */ }
...
router.get('/me', authenticateToken, (req: Request, res: Response): void => {
  // authenticateToken ya validó y cargó req.user
  res.json({ success: true, user: (req as any).user })
})*/
router.get('/me', (req, res, next) => { console.log('HIT /api/auth/me'); next(); }, authenticateToken, (req, res) => {
  res.json({ success: true, user: (req as any).user })
})
